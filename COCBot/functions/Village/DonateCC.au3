			...
			If $DonateTroop Then
				...
				Else
					If $ichkExtraAlphabets = 1 Then
						...
						SetLog(getLocaleString("logChatRequest") & $tempClip)
					Else
...
Func DonateTroopType($Type, $quant = 8, $custom = False)
	...
	For $i = 0 To UBound($TroopName) - 1
		If Eval("e" & $TroopName[$i]) = $Type Then
			...
			If $sDonateQuantityAvaible < 1 Then
				Setlog(getLocaleString("logSorryChief") & NameOfTroop($Type) & getLocaleString("logSorryChiefDontFitRemainingSpace"))
				Return
			EndIf
	...
	If _ColorCheck(_GetPixelColor(237 + ($Slot * 82), $DonatePixel[1] - 5 + $YComp), Hex(0x507C00, 6), 10) Or _
		...
		Else
			SetLog(getLocaleString("logDonating") & $sDonateQuantity & " " & NameOfTroop($Type), $COLOR_GREEN)
...
Func RemainingCCcapacity()
	...
	If Not $aTempTotalDonate = "" Then
		...
		Else
			Setlog(getLocaleString("logErrorReadingCastleCap"), $COLOR_RED) ; log if there is read error
	...
	If $aTotalDonateCapacity < 0 Then
		SetLog(getLocaleString("logUnableToReadCastleCap"), $COLOR_RED)
	Else
		SetLog(getLocaleString("logCastleCap") & $aDonatedTroops & "/" & $aCastleTotalCapacity)
	EndIf
