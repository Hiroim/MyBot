...
Func AttackReport()
	...
	If _ColorCheck(_GetPixelColor($aAtkRprtDECheck[0], $aAtkRprtDECheck[1], True), Hex($aAtkRprtDECheck[2], 6), $aAtkRprtDECheck[3]) Then ; if the color of the DE drop detected
		...
		EndIf
		SetLog(getLocaleString("logLootG") & _NumberFormat($iGoldLast) & getLocaleString("logLootE") & _NumberFormat($iElixirLast) & getLocaleString("logLootDE") & _NumberFormat($iDarkLast) & getLocaleString("logLootT") & $iTrophyLast, $COLOR_GREEN)
	Else
		$iGoldLast = getResourcesLoot(333, 289)
		If _Sleep($iDelayAttackReport2) Then Return
		$iElixirLast = getResourcesLoot(333, 328)
	...
	If $iTrophyLast >= 0 Then
		If _ColorCheck(_GetPixelColor($aAtkRprtDECheck2[0], $aAtkRprtDECheck2[1], True), Hex($aAtkRprtDECheck2[2], 6), $aAtkRprtDECheck2[3]) Then
			...
			SetLog(getLocaleString("logBonusG") & _NumberFormat($iGoldLastBonus) & getLocaleString("logLootE") & _NumberFormat($iElixirLastBonus) & getLocaleString("logLootDE") & _NumberFormat($iDarkLastBonus), $COLOR_GREEN)
		Else
			If _Sleep($iDelayAttackReport2) Then Return
			$iGoldLastBonus = getResourcesBonus(590, 340)
			$iGoldLastBonus = StringReplace($iGoldLastBonus, "+", "")
			...
			$iElixirLastBonus = StringReplace($iElixirLastBonus, "+", "")
			$iDarkLastBonus = 0
			SetLog(getLocaleString("logBonusG") & _NumberFormat($iGoldLastBonus) & getLocaleString("logLootE") & _NumberFormat($iElixirLastBonus), $COLOR_GREEN)
		EndIf
		$LeagueShort = "--"
		For $i = 0 To 15
			If _Sleep($iDelayAttackReport2) Then Return
